...
export default function Lista({ itens, ver: inicial }: { itens: Item[]; ver?: string }) {
  ...
  const [aberta, setAberta] = useState(false); // fechada por padrão; a escolha fica no localStorage

  useEffect(() => {
    try {
      setAberta(localStorage.getItem("esteira_aberta") === "1");
    } catch {}
  }, []);
  ...
  function abreFecha() {
    ...
    if (!v) sair();
    try {
      localStorage.setItem("esteira_aberta", v ? "1" : "0");
    } catch {}
  }
  ...
  return (
    <div className="card" style={{ marginTop: 18, padding: 0 }}>
      <button
        onClick={abreFecha}
        className="row"
        style={{ width: "100%", padding: 12, justifyContent: "space-between", flexWrap: "nowrap", font: "inherit", background: "none", border: "none", cursor: "pointer", color: "var(--ink)" }}
      >
        <span style={{ fontSize: 16, fontWeight: 700 }}>Carrosséis ({itens.length})</span>
        <span className="muted" style={{ fontSize: 18, lineHeight: 1 }}>{aberta ? "⌃" : "⌄"}</span>
      </button>
      {aberta && (
        <>
          <div className="row" style={{ padding: 12, borderTop: "1px solid var(--line)", justifyContent: "space-between", flexWrap: "nowrap" }}>
